...
| 插件（暂名） | 职责 | 对应 Phase |
|------|------|-----------|
| `dsh-plugin-warroom-evidence` | 从 session log 派生一页证据报告（C2 `redteam.*` 事件 → OWASP/ATLAS 映射） | Phase 2 |
| `dsh-plugin-warroom-redloop` | attacker/judge 优化回路（Crescendo + 预算熔断 + `redteam.strategy.adjusted`） | Phase 3 |
